...
ir_brut <- 
  read_dta(here("data/NGIR7BFL_mini.DTA"),
                  col_select = c(
                    ...
                    s1108ba, s1108bb, s1108bc, s1108bd, s1108be 
...
ir_renomme <- 
  ...
  rename(mois_entrevue = v006,
         ...
         palu_mortel = s1108bb,
         palu_facile = s1108bc,
         palu_connait_malade = s1108bd,
         palu_enfant_faible = s1108be 
         )
```
...
ir_propre <- ir_renomme %>% 
  zap_labels() %>%   # supprime labels DHS
...
ggplot(ir_propre, aes(x = palu_mortel, y = age, fill = palu_mortel)) +
  geom_boxplot() +
  labs(title = "Âge selon perception : le paludisme peut entraîner la mort",
       x = "Perception du paludisme mortel",
       y = "Âge")
```
...
ggplot(ir_propre, aes(x = niveau_education)) +
  ...
  labs(title = "Niveau d'éducation des répondantes",
...
ggplot(ir_propre, aes(x = niveau_education, fill = palu_gueri)) +
  geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent) +
  labs(title = "Connaissances : paludisme peut être guéri",
       x = "Niveau d'éducation", y = "Pourcentage")
```
...
ggplot(ir_propre, aes(x = urbain_rural, fill = palu_connait_malade)) +
  geom_bar(position = "dodge") +
  labs(title = "Connaissances palu selon milieu de résidence",
       x = "Milieu", y = "Effectif")
```
...
```{r}
ir_propre %>%
  count(region, palu_mortel) %>%
  ggplot(aes(x = 2, y = n, fill = palu_mortel)) +
  geom_col(width = 1) +
  coord_polar(theta = "y") +
  facet_wrap(~region) +
  theme_void() +
  labs(title = "Perception du caractère mortel du paludisme par région")
```
